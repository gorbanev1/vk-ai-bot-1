...
const sample = [
    ...
    { DetectorName: 'Groq', SecretParts: { key: 'TEST_GROQ_KEY_REDACTED' } },
    { DetectorName: 'OpenAI', SecretParts: { key: 'TEST_OPENAI_KEY_REDACTED' } },
...
test('bot-native import writes numbered slots and injects them into current process.env', () => {
    ...
    writeFileSync(join(root, '.env'), 'OPENAI_API_KEY=TEST_OPENAI_KEY_REDACTED\nNVIDIA_API_KEY=nvapi-primary-000000000000000\n', 'utf8');
    ...
    try {
        ...
        process.env.OPENAI_API_KEY = 'TEST_OPENAI_KEY_REDACTED';
        ...
        assert.ok(process.env.GROQ_API_KEY_1?.startsWith('gsk_'));
        assert.ok(process.env.OPENAI_API_KEY_2?.startsWith('sk-proj-imported'));
        assert.equal(process.env.OPENAI_API_KEY, 'TEST_OPENAI_KEY_REDACTED');
