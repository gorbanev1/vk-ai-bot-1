...
test('V110 exposes owner post-event broadcast and review commands', () => {
...
test('V110 feedback state arms delivered endpoint and consumes exactly one review', () => {
...
test('V110 application sends template through coords recipient endpoints and forwards feedback to owner', () => {
...
test('V110 sends donation QR strictly as a separate second message for coords, thanks and broadcast', () => {
    assert.match(applicationSource, /PERESOZDANIE_DONATION_QR_PATH/u);
    assert.match(applicationSource, /await context\.send\(settings\.messageText\);[\s\S]{0,250}await sendPeresozdanieQrToContext\(context\);/u);
    assert.match(applicationSource, /peresozdanie-thank[\s\S]{0,2500}await sendPeresozdanieQrToContext\(context\);/u);
    assert.match(applicationSource, /await sendCoordsBroadcastToRecipient\(recipient, messageText\);[\s\S]{0,1000}await sendPeresozdanieQrToRecipient\(recipient\);/u);
    assert.match(applicationSource, /QR-код убран из текущей рассылки/u);
    assert.match(applicationSource, /Ну что, был на «Случайном Пересоздании»\? Как тебе\?/u);
    assert.doesNotMatch(applicationSource, /Можно буквально пару слов/u);
});
